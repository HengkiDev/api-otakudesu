...
// Special VT TikTok downloader endpoint
app.get('/api/vttiktok', async (req, res) => {
  try {
    const url = req.query.url;
    ...
    // Validate that the URL is a VT TikTok URL
    if (!url.includes('vt.tiktok.com')) {
      return res.status(400).json({
        status: false,
        message: "This endpoint is specifically for vt.tiktok.com URLs"
      });
    }
    ...
    try {
      ...
      if (redirectError.response && redirectError.response.headers.location) {
        fullUrl = redirectError.response.headers.location;
      }
    }
    
    // Make request to the external API
    const response = await axios.get(`https://api.siputzx.my.id/api/tiktok?url=${encodeURIComponent(fullUrl)}`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
      }
    });
